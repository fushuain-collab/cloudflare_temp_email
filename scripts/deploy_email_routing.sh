...
for z in "$ZONE_ASIA" "$ZONE_KDNS"; do
  ...
  if [ "$ok" != "True" ]; then
    ...
    if [ -n "$rid" ]; then
      r=$(curl -s -X PUTP-H "Authorization: Bearer $CLOUDFLARE_API_TOKEN" -H "Content-Type: application/json" "https://api.cloudflare.com/client/v4/zones/$z/email/routing/rules/$rid" -d '{"name":"Catch-All to Temp Mail Worker","enabled":true,"priority":0,"type":"catch_all","matchers":[{"type":"all"}],"actions":[{"type":"worker","value":[{"worker_name":"$WORKER_NAME"}]}]}')
